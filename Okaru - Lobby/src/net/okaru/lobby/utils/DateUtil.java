...
public class DateUtil {
	...
	public static long parseDateDiff(String time, boolean future)
		...
		if (!found) {
			...
			max.add(1, 10);
			return var13.after(max) ? max.getTimeInMillis() : var13
					.getTimeInMillis();
		}
	}
	...
	static int dateDiff(int type, Calendar fromDate, Calendar toDate,
		...
		long savedDate;
		for (savedDate = fromDate.getTimeInMillis(); future
				&& !fromDate.after(toDate) || !future
				&& !fromDate.before(toDate); ++diff) {
	...
	public static String formatDateDiff(Calendar fromDate, Calendar toDate) {
		...
		if (toDate.equals(fromDate)) {
			...
			String[] names = new String[] { "y ", "y ", "m ", "m ", "d ", "d ",
					"h ", "h ", "m ", "m ", "s ", "s " };
			int accuracy = 0;
			...
			return sb.length() == 0 ? "Refreshing..." : sb.toString().trim();
		}
	}
	...
	public static String formatSimplifiedDateDiff(Calendar fromDate,
			Calendar toDate) {
		boolean future = false;
		if (toDate.equals(fromDate)) {
			return "now";
			...
			StringBuilder sb = new StringBuilder();
			int[] types = new int[] { 1, 2, 5, 11, 12, 13 };
			String[] names = new String[] { "y", "y", "m", "m", "d", "d", "h",
					"h", "m", "m", "s", "s" };
			int accuracy = 0;
	...
	public static String readableTime(BigDecimal time) {
		...
		if (time.doubleValue() <= 60) {
			...
			return text + " " + formatter.format(minutes) + ":"
					+ formatter.format(seconds) + "m";
		}
