...
public class TimerSidebarProvider implements SidebarProvider
{
	...
	public List<SidebarEntry> getLines(Player player)
	{
		...
		lines.add(new SidebarEntry(ChatColor.translateAlternateColorCodes('&', "&4&lKills:&6 " + player.getStatistic(Statistic.PLAYER_KILLS))));
		lines.add(new SidebarEntry(ChatColor.translateAlternateColorCodes('&', "&4&lDeaths:&6 " + player.getStatistic(Statistic.DEATHS))));

		if (player.hasPermission("common.vanish"))
		{
			lines.add(new SidebarEntry(C.c("&e&lStaff Mode:")));
			lines.add(new SidebarEntry(ChatColor.YELLOW + " » " + "Va", "nish: " + (VanishHandler.isVanished(player) ? C.c("&a") + "On" : C.c("&c") + "Off"), null));
			lines.add(new SidebarEntry(ChatColor.YELLOW + " » " + "Game", "mode: " + ChatColor.GREEN + StringUtils.capitalise(player.getGameMode().name().toLowerCase()), null));
		}
