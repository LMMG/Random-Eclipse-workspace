...
public class TeamCommand implements CommandExecutor
{
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (args[0].equalsIgnoreCase("setspawn")) {
            ...
            String team = args[1];
            if (Team.fromString(team) == null) {
                team = (Bunkers.getInstance().getGameHandler().getPlayers().containsKey(team) ? StringUtils.capitalize(Bunkers.getInstance().getGameHandler().getPlayers().get(Bukkit.getPlayer(team).getName()).toString().toLowerCase()) : "NULLBLOCK");
            }
            ...
            String[] members;
            for (int length = (members = Bunkers.getInstance().getTeamManager().getMembers(tem)).length, i = 0; i < length; ++i) {
                final String s = members[i];
                toSend = String.valueOf(toSend) + "§7[" + (Bunkers.getInstance().getGameHandler().getPlayersDeadOrRespawning().contains(Bukkit.getPlayer(s).getName()) ? "§c" : "§a") + s + "§7]" + "\n";
            }
            toSend = String.valueOf(toSend) + " \n";
            toSend = String.valueOf(toSend) + "§aTeam info" + tem.getColor().toString() + ":\n";
            toSend = String.valueOf(toSend) + "§aTotal team balance" + tem.getColor().toString() + ": " + Bunkers.getInstance().getTeamManager().getTeamBalanceFormatted(tem) + "\n";
            toSend = String.valueOf(toSend) + "§aTotal team kills" + tem.getColor().toString() + ": " + Bunkers.getInstance().getTeamManager().getTotalKills(tem) + "\n";
            toSend = String.valueOf(toSend) + "§aTotal team deaths" + tem.getColor().toString() + ": " + Bunkers.getInstance().getTeamManager().getTotalDeaths(tem) + "\n";
            toSend = String.valueOf(toSend) + "§aTotal team points" + tem.getColor().toString() + ": " + Bunkers.getInstance().getTeamManager().getTeamPoints(tem) + "\n";
            toSend = String.valueOf(toSend) + "§aDTR" + tem.getColor().toString() + ": " + Bunkers.getInstance().getDTRManager().getDTRFormatted(tem) + "\n";
            toSend = String.valueOf(toSend) + "§7§m------------------------------------\n";
            ...
            final Selection s2 = Bunkers.getInstance().getWorldEdit().getSelection(player);
            if (s2 == null) {
            ...
            if (Bunkers.getInstance().getChatManager().isTeamChat(player)) {
                Bunkers.getInstance().getChatManager().setTeamChat(player, false);
                player.sendMessage("§aYou are no longer talking in team chat.");
            }
            else {
                Bunkers.getInstance().getChatManager().setTeamChat(player, true);
