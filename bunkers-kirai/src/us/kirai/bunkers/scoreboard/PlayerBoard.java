package us.kirai.bunkers.scoreboard;
...
public class PlayerBoard
{
    ...
    public PlayerBoard(final Bunkers plugin, final Player player) {
        ...
        (this.red = this.scoreboard.registerNewTeam("red")).setPrefix(us.kirai.bunkers.game.Team.RED.getColor().toString());
        this.red.setCanSeeFriendlyInvisibles(true);
        (this.green = this.scoreboard.registerNewTeam("green")).setPrefix(us.kirai.bunkers.game.Team.GREEN.getColor().toString());
        this.green.setCanSeeFriendlyInvisibles(true);
        (this.blue = this.scoreboard.registerNewTeam("blue")).setPrefix(us.kirai.bunkers.game.Team.BLUE.getColor().toString());
        this.blue.setCanSeeFriendlyInvisibles(true);
        (this.yellow = this.scoreboard.registerNewTeam("yellow")).setPrefix(us.kirai.bunkers.game.Team.YELLOW.getColor().toString());
        ...
        new BukkitRunnable() {
            public void run() {
                ...
                for (final Player update : updates) {
                    ...
                    if (PlayerBoard.this.plugin.getGameHandler().getPlayers().get(update.getName()).equals(us.kirai.bunkers.game.Team.RED)) {
                        PlayerBoard.this.red.addPlayer((OfflinePlayer)update);
                    }
                    else if (PlayerBoard.this.plugin.getGameHandler().getPlayers().get(update.getName()).equals(us.kirai.bunkers.game.Team.GREEN)) {
                        PlayerBoard.this.green.addPlayer((OfflinePlayer)update);
                    }
                    else if (PlayerBoard.this.plugin.getGameHandler().getPlayers().get(update.getName()).equals(us.kirai.bunkers.game.Team.BLUE)) {
                        PlayerBoard.this.blue.addPlayer((OfflinePlayer)update);
                    }
                    else {
                        if (!PlayerBoard.this.plugin.getGameHandler().getPlayers().get(update.getName()).equals(us.kirai.bunkers.game.Team.YELLOW)) {
                            continue;
                        }
