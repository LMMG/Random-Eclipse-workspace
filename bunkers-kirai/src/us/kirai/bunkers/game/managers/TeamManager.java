...
public class TeamManager
{
    ...
    public double getTeamBalance(final Team t) {
        ...
        for (final Player p : Bukkit.getOnlinePlayers()) {
            for (final String s : Bunkers.getInstance().getGameHandler().getPlayers().keySet()) {
                if (p.getName().equalsIgnoreCase(s) && Bunkers.getInstance().getGameHandler().getPlayers().get(s).equals(t)) {
                    totalBal += Bunkers.getInstance().getBalanceManager().getBalance(p);
                }
            }
        }
        return totalBal;
    ...
    public double getTeamPoints(final Team t) {
        ...
        for (final Player p : Bukkit.getOnlinePlayers()) {
            for (final String s : Bunkers.getInstance().getGameHandler().getPlayers().keySet()) {
                if (p.getName().equalsIgnoreCase(s) && Bunkers.getInstance().getGameHandler().getPlayers().get(s).equals(t)) {
                    totalBal += Bunkers.getInstance().getPointManager().getPoints(p);
                }
            }
        }
        return totalBal;
