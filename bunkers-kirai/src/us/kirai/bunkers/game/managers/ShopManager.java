...
public class ShopManager implements Listener
{
    ...
    private void buyItem(final Player p, final ItemStack i) {
        ...
        if (Bunkers.getInstance().getBalanceManager().hasEnoughMoney(p, this.getPrice(i))) {
            if (this.getActualItem(i).getType().equals(ShopItemType.ITEM)) {
                ...
                Bunkers.getInstance().getBalanceManager().removeBalance(p, this.getPrice(i));
            }
            else {
                ItemStack[] contents;
        ...
        for (final ItemStack x : items) {
            if (x.getType().equals((Object)Material.GOLD_INGOT)) {
                for (int j = 0; j < x.getAmount(); ++j) {
                    Bunkers.getInstance().getBalanceManager().addBalance(p, 15.0);
                }
                p.getInventory().remove(x);
            }
            if (x.getType().equals((Object)Material.IRON_INGOT)) {
                for (int j = 0; j < x.getAmount(); ++j) {
                    Bunkers.getInstance().getBalanceManager().addBalance(p, 10.0);
                }
                p.getInventory().remove(x);
            }
            if (x.getType().equals((Object)Material.DIAMOND)) {
                for (int j = 0; j < x.getAmount(); ++j) {
                    Bunkers.getInstance().getBalanceManager().addBalance(p, 20.0);
                }
                p.getInventory().remove(x);
            }
        }
    ...
    public void click(final InventoryClickEvent e) {
        if (!Bunkers.getInstance().getGameHandler().getGameState().equals(GameState.GAME)) {
            return;
        }
