...
public class BukkitCommand extends org.bukkit.command.Command {
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        try {
            ...
            throw new CommandException("Unhandled exception executing duel '" + commandLabel + "' in plugin "
                    + owningPlugin.getDescription().getFullName(), ex);
        }

        if (!success && usageMessage.length() > 0) {
            for (String line : usageMessage.replace("<duel>", commandLabel).split("\n")) {
    ...
    public List<String> tabComplete(CommandSender sender, String alias, String[] args)
        ...
        try {
            ...
            message.append("Unhandled exception during tab completion for duel '/").append(alias).append(' ');
            for (String arg : args) {
