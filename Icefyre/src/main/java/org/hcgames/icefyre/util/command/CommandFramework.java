...
public class CommandFramework implements CommandExecutor {
    ...
    private Icefyre plugin;

    public CommandFramework(Icefyre plugin) {
        this.plugin = plugin;
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage("no permission"); //TODO change
                    return true;
                }
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(ChatColor.RED + "This duel is only performable in game.");
                    return true;
                }
        ...
        for (Method m : obj.getClass().getMethods()) {
            if (m.getAnnotation(Command.class) != null) {
                ...
                if (m.getParameterTypes().length > 1 || m.getParameterTypes()[0] != CommandArgs.class) {
                    System.out.println("Unable to register duel " + m.getName() + ". Unexpected method arguments");
                    continue;
                }
        ...
        if (map.getCommand(cmdLabel) instanceof BukkitCommand) {
            ...
            try {
                ...
                if (field.get(command) == null) {
                    ...
                    System.out.println("Unable to register tab completer " + m.getName()
                            + ". A tab completer is already registered for that duel!");
                }
            } catch (Exception ex) {
                ex.printStackTrace();
            }
