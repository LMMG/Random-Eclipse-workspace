package net.nersa.kitmap.eventgame.tracker;
...
public class KothTracker implements EventTracker {
	...
	public void tick(EventTimer eventTimer, EventFaction eventFaction) {
		...
		if (remainingSeconds > 0 && remainingSeconds % 30 == 0) {
			Bukkit.broadcastMessage(ChatColor.GOLD + "[" + eventFaction.getEventType().getDisplayName() + "] " + ChatColor.YELLOW + "Someone is controlling " + ChatColor.RED + captureZone.getDisplayName() + ChatColor.YELLOW + ". " + ChatColor.RED + "§7§o(" + DateTimeFormats.KOTH_FORMAT.format(remainingMillis) + ')');
		}
	}

	@Override
	public void onContest(EventFaction eventFaction, EventTimer eventTimer) {
		Bukkit.broadcastMessage(ChatColor.GOLD + "[" + eventFaction.getEventType().getDisplayName() + "] " + ChatColor.RED + eventFaction.getName() + ChatColor.YELLOW + " can now be contested. " + ChatColor.RED + "§7§o(" + DateTimeFormats.KOTH_FORMAT.format(eventTimer.getRemaining()) + ')');
	}
	...
	public void onControlLoss(Player player, CaptureZone captureZone, EventFaction eventFaction) {
		...
		if (remainingMillis > 0L && captureZone.getDefaultCaptureMillis() - remainingMillis > MINIMUM_CONTROL_TIME_ANNOUNCE) {
			Bukkit.broadcastMessage(ChatColor.GOLD + "[" + eventFaction.getEventType().getDisplayName() + "] " + ChatColor.RED + captureZone.getDisplayName() + ChatColor.YELLOW + " has been knocked!" + " §7§o(" + captureZone.getScoreboardRemaining() + ')');
		}
	}
