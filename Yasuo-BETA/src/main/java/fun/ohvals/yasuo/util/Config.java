...
public class Config {
    ...
    public Config(String name) {
        file = new File(Yasuo.getYasuo().getDataFolder() + File.separator, name + ".yml");
        ...
        if (!file.exists()) {
            file = loadResource(yasuo, name + ".yml");
        }

        try {
            config = ConfigurationProvider.getProvider(YamlConfiguration.class).load(file);
        } catch (IOException e) {
            System.out.println(ChatColor.RED + "[" + yasuo.getDescription().getName() + "" + yasuo.getDescription().getVersion() + "] was not able to load '" + file.getName() + "'. Printing stacktrace below..." + ChatColor.RESET);
            e.printStackTrace();
        }
    }
    ...
    public void save() {
        try {
            ...
            System.out.println(ChatColor.RED + "[" + yasuo.getDescription().getName() + "" + yasuo.getDescription().getVersion() + "] was not able to save '" + file.getName() + "'. Printing stacktrace below..." + ChatColor.RESET);
            e.printStackTrace();
        }
    }
