		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(Player player : Bukkit.getOnlinePlayers()) {
					...
					if(g.getName().equalsIgnoreCase("Wenja")) {
						playerNames.add(player.getName());
					}
				}
				Bukkit.broadcastMessage("");
				Bukkit.broadcastMessage(ChatColor.GOLD + "Wenja Users " + ChatColor.GRAY + "» " + ChatColor.WHITE + playerNames.toString());
				Bukkit.broadcastMessage(ChatColor.GRAY + "This can be purchased on our store store.wenjapvp.net!");
				Bukkit.broadcastMessage("");
			}
