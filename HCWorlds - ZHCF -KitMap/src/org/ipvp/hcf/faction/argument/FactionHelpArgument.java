    ...
    private void showPage(CommandSender sender, String label, int pageNumber) {
        ...
        if (pages == null) {
            ...
            for (CommandArgument argument : executor.getArguments()) {
                ...
                count++;
                pages.get(val).add(ChatColor.YELLOW + "/" + label + ' ' + argument.getName() + ChatColor.GOLD + " » " + ChatColor.WHITE + argument.getDescription());
        ...
        sender.sendMessage(ChatColor.YELLOW + " Faction Help " + ChatColor.WHITE + "(Page " + pageNumber);
        for (String message : pages.get(pageNumber)) {
            sender.sendMessage("  " + message);
        }

        sender.sendMessage(ChatColor.YELLOW + " You are currently on " + ChatColor.GOLD + "Page " + pageNumber);
        sender.sendMessage(ChatColor.YELLOW + " To view other pages, use " + ChatColor.GOLD + '/' + label + ' ' + getName() + " <page#>" + ChatColor.GOLD + '.');
        sender.sendMessage(ChatColor.GOLD + BukkitUtils.STRAIGHT_LINE_DEFAULT);
    }
}
