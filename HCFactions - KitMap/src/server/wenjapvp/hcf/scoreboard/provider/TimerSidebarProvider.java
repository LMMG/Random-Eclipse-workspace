...
public class TimerSidebarProvider implements SidebarProvider
{
	...
	{
		return ChatColor.GREEN + ChatColor.BOLD.toString() + "Mist" + ChatColor.GRAY + " [Season 1]";
	}
	...
	public List<SidebarEntry> getLines(Player player)
	{
		...
		if(player.hasPermission("common.vanish")) {
			lines.add(new SidebarEntry(C.c("&a&lStaff Mode:")));
			lines.add(new SidebarEntry(ChatColor.GRAY + " » " +"Va", "nish: " + (VanishHandler.isVanished(player) ? C.c("&c") + "On" : C.c("&a") + "Off"), null));
			lines.add(new SidebarEntry(ChatColor.GRAY +  " » " + "Game","mode: " + ChatColor.GREEN + StringUtils.capitalise(player.getGameMode().name().toLowerCase()), null));
		}
