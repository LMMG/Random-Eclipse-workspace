package net.nersa.lobby.manager;
...
public class CommandManager implements Listener
{
	...
	{
		final String[] withArguements = e.getMessage().split(" ");
		final String command = withArguements[0].substring(1);
		if(command.contains("eban")) {
			e.setCancelled(true);
	...
	{
		Bukkit.getLogger().severe("msg: " + event.getMessage());
		if ((event.getMessage().startsWith("/zperm")) && (!event.getPlayer().hasPermission("wenja.perms")))
		{
			event.setCancelled(true);
			event.getPlayer().sendMessage("Unknown command. Type \"/help\" for help.");
		}
