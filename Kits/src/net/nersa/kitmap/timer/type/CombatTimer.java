...
import net.nersa.kitmap.timer.event.TimerClearEvent;
import net.nersa.kitmap.timer.event.TimerStartEvent;
...
public class CombatTimer extends PlayerTimer implements Listener {
	...
	public CombatTimer(HCF plugin) {
		super("Combat", TimeUnit.SECONDS.toMillis(29L));
		this.plugin = plugin;
	}
	...
	public void onTimerStart(TimerStartEvent event) {
		if (event.getTimer() == this) {
			...
			if (optional.isPresent()) {
				Player player = optional.get();
				player.sendMessage(ChatColor.YELLOW + "You are now Spawn Tagged for " + ChatColor.YELLOW + DurationFormatUtils.formatDurationWords(event.getDuration(), true, true) + ChatColor.YELLOW + '.');
			}
		}
	}
