package net.nersa.kitmap.pvpclass.bard;
...
public class BardClass extends PvpClass implements Listener {
	...
	public BardClass(HCF plugin) {
		super("Bard", TimeUnit.SECONDS.toMillis(1));
		this.plugin = plugin;
	...
	public void onPlayerInteract(final PlayerInteractEvent event) {
		...
		if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
			...
			if (bardData != null) {
				...
				player.sendMessage(ChatColor.GOLD + "You have just used " + name + " buff " + ChatColor.RED + Lang.fromPotionEffectType(bardEffect.clickable.getType()) + ' ' + (bardEffect.clickable.getAmplifier() + 1) + ChatColor.GOLD + " costing you " + ChatColor.BOLD + bardEffect.energyCost + ChatColor.YELLOW + " energy. " + "Your energy is now " + ChatColor.GREEN + ((newEnergy * 10.0) / 10.0)/* TODO:neeeded? */ + ChatColor.YELLOW + '.');
			}
		}
	}
