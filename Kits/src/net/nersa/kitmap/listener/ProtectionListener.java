...
import net.nersa.kitmap.faction.type.WarzoneFaction;
...
public class ProtectionListener implements Listener {
	...
	public static boolean attemptBuild(Entity entity, Location location, @Nullable String denyMessage, boolean isInteraction) {
		...
		if (player != null && factionAt instanceof PlayerFaction) {
			if (HCF.getInstance().getFactionManager().getPlayerFaction(player) == factionAt) {
				result = true;
			}
		}
	...
	public static boolean canBuildAt(Location from, Location to) {
		Faction toFactionAt = HCF.getInstance().getFactionManager().getFactionAt(to);
		return !(toFactionAt instanceof Raidable && !((Raidable) toFactionAt).isRaidable() && toFactionAt != HCF.getInstance().getFactionManager().getFactionAt(from));
	}
