    ...
    private void showPage(CommandSender sender, String label, int pageNumber) {
        ...
        if (pages == null) {
            ...
            for (CommandArgument argument : executor.getArguments()) {
                ...
                count++;
                pages.get(val).add(ChatColor.GREEN + "/" + label + ' ' + argument.getName() + ChatColor.GRAY + " - " + ChatColor.GREEN + argument.getDescription());
        ...
        sender.sendMessage(ChatColor.GREEN + ChatColor.BOLD.toString() + " Faction Help " + ChatColor.GRAY + "- " + ChatColor.GREEN + "(Page " + pageNumber + '/' + totalPageCount + ')');
        for (final String message : this.pages.get(Integer.valueOf(pageNumber))) {
            sender.sendMessage("  " + message);
        }
        sender.sendMessage(ChatColor.GREEN + ChatColor.BOLD.toString() + " * " + ChatColor.GRAY + "You are currently on " + ChatColor.GREEN + "Page " + pageNumber + '/' + totalPageCount + ChatColor.GREEN + '.');
        sender.sendMessage(ChatColor.GREEN + ChatColor.BOLD.toString() + " * " + ChatColor.GRAY + "To view other pages, use " + ChatColor.GREEN + '/' + label + ' ' + this.getName() + " <page#>" + ChatColor.GREEN + '.');
        sender.sendMessage(ChatColor.GRAY + BukkitUtils.STRAIGHT_LINE_DEFAULT);
    }
}
