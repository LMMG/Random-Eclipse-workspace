...
public class TimerSidebarProvider implements SidebarProvider
{
	...
	{
		return ChatColor.DARK_GREEN + ChatColor.BOLD.toString() + "Mist HCF" + ChatColor.RED + " [Map I]";
	}
	...
	public List<SidebarEntry> getLines(Player player)
	{
		...
		{
			lines.add(new SidebarEntry(C.c("&2&lStaff Mode:")));
			lines.add(new SidebarEntry(ChatColor.GRAY + " » " + "Online: " + ChatColor.DARK_GREEN + String.valueOf(Bukkit.getOnlinePlayers())));
			lines.add(new SidebarEntry(ChatColor.GRAY + " » " + "Va", "nish: " + (VanishHandler.isVanished(player) ? C.c("&a") + "On" : C.c("&c") + "Off"), null));
			lines.add(new SidebarEntry(ChatColor.GRAY + " » " + "Game", "mode: " + ChatColor.DARK_GREEN + StringUtils.capitalise(player.getGameMode().name().toLowerCase()), null));
		}
		...
		else if (eventFaction instanceof ConquestFaction)
		{
			...
			for (final Map.Entry<PlayerFaction, Integer> entry : conquestTracker.getFactionPointsMap().entrySet())
			{
				...
				conquestLines.add(new SidebarEntry(ChatColor.GOLD, " ï¿½" + ChatColor.RED + factionName, ChatColor.GRAY + ": " + ChatColor.WHITE + entry.getValue()));
				if (++count == 3)
				{
					break;
				}
