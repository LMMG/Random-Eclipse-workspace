...
public class PlayerTimer
{
	...
	{
		if (cooldown.containsKey(k)) { throw new IllegalArgumentException("Cooldown already exists."); }
		cooldown.put(k, new HashMap());
	}

	public static HashMap<UUID, Long> getCooldownMap(String k)
	{
		if (cooldown.containsKey(k)) { return (HashMap) cooldown.get(k); }
		return null;
	}

	public static void addCooldown(String k, Player p, int seconds)
	{
		if (!cooldown.containsKey(k)) { throw new IllegalArgumentException(k + " does not exist"); }
	...
	{
		return (cooldown.containsKey(k)) && (((HashMap) cooldown.get(k)).containsKey(uuid)) && (System.currentTimeMillis() <= ((Long) ((HashMap) cooldown.get(k)).get(uuid)).longValue());
	}
