...
import com.hcempire.horus.profile.fight.killer.ProfileFightKiller;
...
import com.hcempire.horus.util.NumberUtil;
...
public class DeathLookup {
    ...
    public long getDeathsCount() {
        return main.getHorusDatabase().getFights().count(eq("killed", profile.getUuid().toString())) + profile.getFights().size();
    }
    ...
    public List<ProfileFight> getDeaths(int page) {
        ...
        MongoCursor cursor = main.getHorusDatabase().getFights().find(eq("killed", profile.getUuid().toString())).skip((page - 1) * PAGE_SIZE).limit(PAGE_SIZE - toLimit).sort(descending("occurred_at")).iterator();
