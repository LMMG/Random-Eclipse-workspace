            ...
            public void run() {
                for (Player player : Bukkit.getOnlinePlayers()) {
                    ...
                    if (profile.getKit() == null) {
                        ...
                        if (kitWarmup != null) {
                            if (!(kitWarmup.getKit().isWearingArmor(player))) {
                                ...
                                player.sendMessage(main.getLangFile().getString("KIT.CANCELLED").replace("%KIT%", kitWarmup.getKit().getName()));
                            } else {
                                if (kitWarmup.getTimeLeft().equalsIgnoreCase("0.1")) {
                                    ...
                                    player.sendMessage(main.getLangFile().getString("KIT.APPLIED").replace("%KIT%", kitWarmup.getKit().getName()));
                                }
                            }
                            continue;
                        }
                        ...
                        for (ProfileKit kit : ProfileKit.values()) {
                            ...
                            if (kit.isWearingArmor(player)) {
                                if (main.getConfigFile().getBoolean("KIT.WARMUP.ENABLED")) {
                                    profile.setKitWarmup(new ProfileKitWarmup(kit, ProfileKitWarmup.DEFAULT_DURATION));
                                    player.sendMessage(main.getLangFile().getString("KIT.WARMUP").replace("%KIT%", kit.getName()));
                                } else {
                                    profile.setKit(kit);
                                    if (kit == ProfileKit.BARD) {
                                        profile.setEnergy(new ProfileKitEnergy());
                                    }
                                    player.sendMessage(main.getLangFile().getString("KIT.APPLIED").replace("%KIT%", kit.getName()));
                                }
                                break;
                            }
                        }
                    ...
                    if (!(profile.getKit().isWearingArmor(player))) {
                        ...
                        player.sendMessage(main.getLangFile().getString("KIT.REMOVED").replace("%KIT%", profile.getKit().getName()));
                        profile.setKit(null);
                    } else {
