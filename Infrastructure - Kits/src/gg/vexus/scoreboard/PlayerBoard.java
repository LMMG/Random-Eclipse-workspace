...
//        this.archerTagged = this.scoreboard.registerNewTeam("archerTagged").setPrefix(ChatColor.DARK_RED.toString());
            ...
            public void run()
            {
                ...
                for (Player update : updates) {
                    ...
                    else
                    {
                        if (!hasRun)
                        {
                            playerFaction = HCF.getPlugin().getFactionManager().getPlayerFaction(PlayerBoard.this.player);
                            hasRun = true;
                        }
                        if (ArcherClass.TAGGED.containsKey(update.getUniqueId()))
                        {
                        ...
                        else
                        {
                            ...
                            if ((playerFaction == null) || ((targetFaction = HCF.getPlugin().getFactionManager().getPlayerFaction(update)) == null))
                            {
	...
	public void addUpdates(Player[] onlinePlayers)
	{
